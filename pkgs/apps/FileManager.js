...
const pkg = {
  ...
  start: async function (Root) {
      ...
      Sidebar.new(sidebarWrapper, [
        ...
        },
        {
          onclick: async (_) => {
            ...
            if (result === false) return;
            result = result.replace(/\//g, "");
            await Vfs.createFile(path + "/" + result);
            renderFileList(path);
          },
            ...
            input.onchange = (e) => {
              ...
              if (
                ...
                reader.onload = async (readerEvent) => {
                  ...
                  await Vfs.createFile(`${path}/${file.name}`);
    ...
    await Vfs.importFS();
    await FileMappings.init(undefined, Root.Core);
      ...
      for (let i = 0; i < fileList.length; i++) {
        ...
        switch (file.type) {
          case "Folder":
            ...
            break;
          case "file":
            alert(mapping.fullName, mapping.label);
    ...
    if (Root.Arguments.data !== undefined) {
      path = Root.Arguments.data.path;
      renderFileList(path);
    } else {
